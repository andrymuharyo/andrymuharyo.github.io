  ...
  >
    <div class="flex flex-col items-center justify-center h-full px-4 sm:px-6">
      ...
      <div class="my-12 px-4 sm:px-6 lg:px-8 lg:container mx-auto">
        <div
          ref="cardRef"
          :class="isCardVisible ? '' : '!opacity-0'"
          class="grid grid-cols-3 lg:flex lg:flex-wrap justify-center gap-20 w-full"
        >
          <div
            v-for="(item, idx) in skillFacts"
            :key="idx"
            :ref="(el) => setSkillItemRef(el as HTMLDivElement | null, idx)"
          ...
          >
            <div class="relative w-24 h-24 flex items-center justify-center group/item">
              ...
              <NuxtImg
                :src="item.icon"
                alt="icon"
                class="w-10 h-10 transition-opacity duration-300 ease-in-out"
              />
            </div>
            <div class="space-y-2">
              <h5 class="mt-3 text-center text-base font-medium text-xl">
                {{ item.label }}
              </h5>
